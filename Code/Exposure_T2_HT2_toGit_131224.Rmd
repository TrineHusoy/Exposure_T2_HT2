...
Occurrence_R <- read_delim("./Data/Results_occurance_T2HT2.csv")
```
...
f_conc_grain <- function(y){
  ...
  for(i in 1:nrow(y)){
    set.seed(123)
    x[[i]] <- rlnorm(n=y$sample_size[i], y$mu[i], y$se[i])
  }
  x <- unlist(x)
...
conc_t2_raw <- conc_t2_raw %>% dplyr::select("type", "raw", "sample_size", "mu", "se", "ref")
...
conc_ht2_raw <- conc_ht2_raw %>% dplyr::select("type", "raw", "sample_size", "mu", "se", "ref")
...
```{r}
# T2
IntakeT2firstday <- f_MC_Sim(Consumption, concentration_foodcat_t2)
IntakeT2firstday <- IntakeT2firstday %>% dplyr::select(Idkode, Day, Time, foodcat, V1:V1000)

# HT2
IntakeHT2firstday <- f_MC_Sim(Consumption, concentration_foodcat_ht2)
IntakeHT2firstday <- IntakeHT2firstday %>% dplyr::select(Idkode, Day, Time, foodcat, V1:V1000)
```

Make a summary table for the markdown document
```{r}
# T2
IntakeT2firstday_sum <- IntakeT2firstday
IntakeT2firstday_sum$mean <- rowMeans(IntakeT2firstday_sum[,5:1004])
IntakeT2firstday_sum$P95 <- apply(IntakeT2firstday_sum[,5:1004],1, quantile, probs = 0.95)
IntakeT2firstday_sum <- IntakeT2firstday_sum %>% dplyr::select("Day", "Time", "foodcat", "mean", "P95")


set_flextable_defaults(font.size = 9, theme_fun = theme_vanilla)
IntakeT2firstday_sum %>% 
  flextable() %>% 
  colformat_double(digits = 0)

# HT2
IntakeHT2firstday_sum <- IntakeHT2firstday
IntakeHT2firstday_sum$mean <- rowMeans(IntakeHT2firstday_sum[,5:1004])
IntakeHT2firstday_sum$P95 <- apply(IntakeHT2firstday_sum[,5:1004],1, quantile, probs = 0.95)
IntakeHT2firstday_sum <- IntakeHT2firstday_sum %>% dplyr::select("Day", "Time", "foodcat", "mean", "P95")


set_flextable_defaults(font.size = 9, theme_fun = theme_vanilla)
IntakeHT2firstday_sum %>% 
  flextable() %>% 
  colformat_double(digits = 0)
```
...
```{r}
write.xlsx(IntakeT2firstday,
           file = file.path(newday, "IntakeT2firstday.xlsx"),
           colNames = TRUE, borders = "rows"
)

write.xlsx(IntakeHT2firstday,
           file = file.path(newday, "IntakeHT2firstday.xlsx"),
           colNames = TRUE, borders = "rows"
)
```
...
# T2
Exp_T2_plot <- IntakeT2firstday %>% dplyr::select(Idkode, foodcat, V1:V1000)
Exp_T2_plot <- Exp_T2_plot %>% tidyr::pivot_longer(cols = c(contains("V")), names_to = "T2",
                                                   values_to = "value")


# HT2
Exp_HT2_plot <- IntakeHT2firstday %>% dplyr::select(Idkode, foodcat, V1:V1000)
