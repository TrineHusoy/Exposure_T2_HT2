...
```{r}
# T2
Intake_T2 <- f_MC_Sim(Consumption, concentration_foodcat_t2)
Intake_T2 <- Intake_T2 %>% dplyr::select(Idkode, Day, Time, foodcat, V1:V1000)

# HT2
Intake_HT2 <- f_MC_Sim(Consumption, concentration_foodcat_ht2)
Intake_HT2 <- Intake_HT2 %>% dplyr::select(Idkode, Day, Time, foodcat, V1:V1000)
```

Make a summary table for the markdown document
```{r}
# T2
Intake_T2_sum <- Intake_T2
Intake_T2_sum$mean <- rowMeans(Intake_T2_sum[,5:1004])
Intake_T2_sum$P95 <- apply(Intake_T2_sum[,5:1004],1, quantile, probs = 0.95)
Intake_T2_sum <- Intake_T2_sum %>% dplyr::select("Day", "Time", "foodcat", "mean", "P95")
...
# HT2
Intake_HT2_sum <- Intake_HT2
Intake_HT2_sum$mean <- rowMeans(Intake_HT2_sum[,5:1004])
Intake_HT2_sum$P95 <- apply(Intake_HT2_sum[,5:1004],1, quantile, probs = 0.95)
Intake_HT2_sum <- Intake_HT2_sum %>% dplyr::select("Day", "Time", "foodcat", "mean", "P95")
...
## Save the T2 and HT2 (ng) Intake file
```{r}
write.xlsx(Intake_T2,
           file = file.path(newday, "Intake_T2.xlsx"),
           colNames = TRUE, borders = "rows"
)

write.xlsx(Intake_HT2,
           file = file.path(newday, "Intake_HT2.xlsx"),
           colNames = TRUE, borders = "rows"
)
```
...
```{r}
# T2
Exp_T2_plot <- Intake_T2 %>% dplyr::select(Idkode, foodcat, V1:V1000)
Exp_T2_plot <- Exp_T2_plot %>% pivot_longer(cols = c(contains("V")), "T2")

# HT2
Exp_HT2_plot <- Intake_HT2 %>% dplyr::select(Idkode, foodcat, V1:V1000)
